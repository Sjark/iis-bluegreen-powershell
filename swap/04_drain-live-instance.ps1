...
Write-Output "Taking ($liveInstance) down gracefully"
Set-InstanceState $serverFarmName $liveInstance 1
Write-Output "$liveInstance is draining connections"
...
# after 10s
$startDate = Get-Date
Write-Output "Checking requests per second"
Do {
    $currentConnections = Get-RequestsPerSecond $serverFarmName $liveInstance

    If ($currentConnections -gt 0) {
        Write-Output "Still $currentConnections requests per second"
    }
    else {
        Write-Output "0 requests per second"
    }
...
Write-Output "$liveInstance is drained (or we timed out waiting), taking down"
Set-ServerOffline $serverFarmName $liveInstance
Write-Output "Old live site is offline"
